...
app.patch("/jokes/:id", (req, res)=>{
  ...
  const newJoke = {
    id: id,
    jokeText: req.body.text || curr.jokeText,
    jokeType: req.body.type || curr.jokeType
  };
  const index = jokes.findIndex((joke)=>joke.id===id);
  jokes[index] = newJoke;
  res.json(newJoke);
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  if(index>-1){
    ...
    res.sendStatus(404).json({error:`Joke with ${id} not found. No jokes have been removed`});
  }
});
//8. DELETE All jokes
app.delete("/jokes/all", (req, res)=>{
  ...
  if(key===masterKey){
    ...
    res.sendStatus(404).json({error: `Key not matched. No jokes have been removed`});
  }
});
app.listen(port, () => {
